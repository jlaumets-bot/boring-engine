...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'generate') {
      ...
      const out = await callLLM({ timeoutMs: 45000, deadlineMs: 50000, messages: [{ role: 'system', content: sys }, { role: 'user', content: usr }], model: 'grok', max_tokens: 600, engine: (bc.engine || 'grok') });   // v689: bounded — this leg is followed by a 50s image call inside one budget
